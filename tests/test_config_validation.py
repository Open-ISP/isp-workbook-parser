...
def test_end_row_runs_into_notes_throws_error():
    ...
    error_message = "The first column of the table DUMMY contains the sub string 'Notes:'."
    with pytest.raises(TableConfigError, match=error_message):
        workbook.get_table_from_config(table_config)
...
def test_end_row_too_soon_throws_error():
    ...
    error_message = "There is data in the row after the defined table end for table DUMMY."
    with pytest.raises(TableConfigError, match=error_message):
        workbook.get_table_from_config(table_config)
...
def test_header_mismatch_throws_error():
    ...
    error_message = (
        "Column names for the table DUMMY don't match the header names provided in the config."
    )
    with pytest.raises(TableConfigError, match=error_message):
        workbook.get_table_from_config(table_config)
