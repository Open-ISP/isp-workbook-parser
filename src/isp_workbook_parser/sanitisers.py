...
def _column_name_sanitiser(columns: pd.Index | pd.Series) -> pd.Index | pd.Series:
    ...
    columns = _replace_newlines_with_whitespace(columns)
    columns = _remove_double_whitespaces(columns)
    columns = _remove_trailing_footnotes_column_names(columns)
    return columns


def _replace_newlines_with_whitespace(
...
def _remove_trailing_footnotes(series: pd.Index | pd.Series) -> pd.Index | pd.Series:
...
def _remove_trailing_footnotes_column_names(
    series: pd.Index | pd.Series,
) -> pd.Index | pd.Series:
    """Removes footnotes by replacing a single trailing digit not preceded by
...
def _remove_double_whitespaces(series: pd.Index | pd.Series) -> pd.Index | pd.Series:
    """Removes any duplicated whitespaces"""
    return series.str.replace(r"\s\s", " ", regex=True)


def _remove_trailing_asterisks(series: pd.Index | pd.Series) -> pd.Index | pd.Series:
    """Replaces trailing asterisks with an empty string"""
    return series.str.replace(r"\*$", "", regex=True)


def _remove_commas(series: pd.Index | pd.Series) -> pd.Index | pd.Series:
